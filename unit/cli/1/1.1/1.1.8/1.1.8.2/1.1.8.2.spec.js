...
it('1.1.8.2 <filename> must end with .spec.md (is appended if missing)', () => {
    ...
    try {
        execSync(`node scripts/cli/index.js create ${folder} ${filenameNoExt} "${title}"`, { encoding: 'utf8' });
        ...
        execSync(`node scripts/cli/index.js create ${folder2} ${filenameWithExt} "${title}"`, { encoding: 'utf8' });
        ...
        expect(fs.existsSync(path.join(folder2, 'another.spec.md.spec.md'))).toBe(false);
