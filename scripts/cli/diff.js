...
export function diffCommand(file, sectionNumber, options = {}) {
    if (options.help) {
        ...
        console.log('');
        console.log('Displays changed specifications for the specified file since the last commit.');
    ...
    let diffOutput = '';
    try {
        diffOutput = execSync(`git diff HEAD --unified=0 "${filePath}"`, { encoding: 'utf8' });
    } catch (e) {
    ...
    const changedSections = targetSections.filter(s => changedLineNumbers.has(s.lineIndex));

    if (changedSections.length === 0) {
        if (options.json) {
            console.log(JSON.stringify([]));
        } else {
            console.log('No changed specification items found in the specified scope.');
        }
        return;
    }
    ...
    if (options.json) {
        ...
        console.log(`Changes in ${file}${sectionNumber ? ` section ${sectionNumber}` : ''}:`);
        console.log('--------------------------------------------------');
