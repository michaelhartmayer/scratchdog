...
export function listCommand(folder = 'specifications', options = {}) {
    if (options.help) {
        ...
        console.log('');
        console.log('Lists all specification files in the specified folder, recursively.');
        console.log('If no folder is provided, it defaults to the "specifications" directory.');
        console.log('');
        console.log('Options:');
    ...
    if (options.json) {
        ...
        specFiles.sort().forEach(filePath => {
            ...
            console.log(`${dir === '.' ? '' : dir + '/'}${name} (${lineCount} spec lines)`);
        });
    }
}
