...
export async function tocCommand(file, sectionNumber, options = {}) {
  if (options.help) {
    console.log('Usage: npm run specter toc <file> [section]');
    console.log('');
    console.log('Displays the table of contents for the specified file.');
    console.log(
      'If [section] is provided, it shows only the top-level sub-sections of that section.',
    );
    console.log('');
    console.log('Options:');
  ...
  if (options.json) {
    ...
    console.log(
      `Table of Contents for ${file}${sectionNumber ? ` section ${sectionNumber}` : ''}:`,
    );
    console.log('--------------------------------------------------');
    ...
    targetSections.forEach((s) => {
      ...
      console.log(
        `${s.number} ${s.text} (${allSubSections.length} spec lines)`,
      );
    });
  }
}
