...
test('8.4.1-8.4.3 Pill visual verification', async ({ page }) => {
  ...
  const borderRadius = await pillSegments
    .first()
  ...
  const boxShadow = await pillSegments
    .first()
    .evaluate((el) => window.getComputedStyle(el).boxShadow);
  const backgroundImage = await pillSegments
    .first()
    .evaluate((el) => window.getComputedStyle(el).backgroundImage);

  const hasShading =
    boxShadow !== 'none' || backgroundImage.includes('gradient');
